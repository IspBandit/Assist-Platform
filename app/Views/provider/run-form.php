...
<section class="section">
    <div class="container">
        <h1><?= $run ? 'Edit run' : 'New run' ?></h1>
        <?php $this->include('partials.provider-nav', ['active' => 'runs']); ?>

        <div class="card">
            <form method="post" action="<?= e(url('provider/runs/save')) ?>" class="stack">
                <?= csrf_field() ?>
                ...
                <div class="grid grid-2">
                    <div class="form-group"><label for="appointments_total">Total places</label><input type="number" min="0" id="appointments_total" name="appointments_total" value="<?= e((string) $v('appointments_total')) ?>"></div>
                ...
                <div class="btn-row">
                    ...
                    <label><input type="checkbox" name="is_public" value="1" <?= $run === null || $v('is_public') ? 'checked' : '' ?>> List publicly</label>
                </div>

                <div class="btn-row">
                    <button type="submit" class="btn btn-primary"><?= $run ? 'Save changes' : 'Create run' ?></button>
                    <a class="btn btn-ghost" href="<?= e(url('provider/runs')) ?>">Cancel</a>
                </div>
            </form>
        </div>
    </div>
