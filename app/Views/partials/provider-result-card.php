...
<article<?= $resultCardId !== '' ? ' id="' . e_attr($resultCardId) . '"' : '' ?> class="provider-card<?= !empty($p['is_featured']) ? ' provider-card--featured' : '' ?><?= $compact ? ' provider-card--compact' : '' ?>"<?= $resultCardId !== '' ? ' tabindex="-1"' : '' ?>>
    ...
    <div class="btn-row provider-card-actions"><a class="provider-card-link" href="<?= e(url($profilePath . $p['slug'])) ?>"><?= $compact ? 'Details' : 'View services and contact details' ?></a><?php if ($canCall): ?><a class="provider-card-link" href="<?= e(url('go/phone/' . $p['slug']) . $contactQuery) ?>">Call</a><?php endif; ?><?php if ($mapDestination !== ''): ?><a class="provider-card-link" href="<?= e(url('go/directions/' . $p['slug']) . $contactQuery) ?>" data-map-directions data-map-destination="<?= e_attr($mapDestination) ?>" target="_blank" rel="noopener noreferrer">Directions</a><?php endif; ?></div>
</article>
