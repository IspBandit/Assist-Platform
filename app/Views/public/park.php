    ...
    <div class="container">
        ...
        <div class="grid grid-2" style="margin-top:1rem;align-items:flex-start">
            ...
            <div class="card stack">
                ...
                <?php if ($park['phone']): ?><p style="margin:0"><strong>Phone:</strong> <a href="<?= e(url('go/stay/phone/' . $park['slug'])) ?>"><?= $this->e((string) $park['phone']) ?></a></p><?php endif; ?>
                <?php if (!empty($park['email'])): ?><p style="margin:0"><strong>Email:</strong> <a href="<?= e(url('go/stay/email/' . $park['slug'])) ?>"><?= $this->e((string) $park['email']) ?></a></p><?php endif; ?>
                <?php if ($park['website']): ?><p style="margin:0"><strong>Website:</strong> <a href="<?= e(url('go/stay/website/' . $park['slug'])) ?>" target="_blank" rel="noopener nofollow"><?= $this->e((string) $park['website']) ?></a></p><?php endif; ?>
                <?php if (!empty($park['booking_url'])): ?><p style="margin:0"><a class="btn btn-secondary" href="<?= e(url('go/stay/booking/' . $park['slug'])) ?>" target="_blank" rel="noopener nofollow">Book this stay</a></p><?php endif; ?>
                <?php if ($mapDestination !== ''): ?><a class="btn btn-primary" href="<?= e(url('go/stay/directions/' . $park['slug'])) ?>" data-map-directions data-map-destination="<?= e_attr($mapDestination) ?>" target="_blank" rel="noopener noreferrer">Directions from my location</a><?php endif; ?>
