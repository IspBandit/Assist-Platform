...
<div class="card">
    ...
    <form method="post" action="<?= e(url('admin/parks/save')) ?>" class="stack" style="margin-top:1rem">
        ...
        <div class="grid grid-2"><div class="form-group"><label for="stay_type">Stay type</label><select id="stay_type" name="stay_type"><?php foreach (['caravan_park'=>'Caravan park','campground'=>'Campground','free_camp'=>'Free camp','national_park'=>'National park camping','showground'=>'Showground','rest_area'=>'Permitted overnight rest area','council_camp'=>'Council camp','farm_stay'=>'Farm stay','station_stay'=>'Station stay','other'=>'Other'] as $v=>$l): ?><option value="<?= e_attr($v) ?>" <?= ($park['stay_type'] ?? '')===$v?'selected':'' ?>><?= $this->e($l) ?></option><?php endforeach; ?></select></div><div class="form-group"><label for="price_type">Cost</label><select id="price_type" name="price_type"><?php foreach (['free'=>'Free','donation'=>'Donation','low_cost'=>'Low cost','paid'=>'Paid','unknown'=>'Unknown'] as $v=>$l): ?><option value="<?= e_attr($v) ?>" <?= ($park['price_type'] ?? '')===$v?'selected':'' ?>><?= $this->e($l) ?></option><?php endforeach; ?></select></div></div>
