...
final class AdvertisingController extends Controller
{
    ...
    public function save(Request $request): Response
    {
        ...
        try{$pdo->beginTransaction();
            ...
            if($id===0){Database::query('INSERT INTO advertisers (business_name,contact_email,status,created_at) VALUES (?,?,\'active\',NOW())',[$business,trim((string)$request->input('contact_email'))?:null]);$advertiserId=(int)$pdo->lastInsertId();Database::query("INSERT INTO advertising_campaigns (advertiser_id,brand_id,name,placement,context_key,status,starts_at,ends_at,destination_url,sponsorship_label,priority,created_at) VALUES (?,?,?,'towsmart_result',?,?,?,?,?,?,?,NOW())",[$advertiserId,(int)$request->input('brand_id'),trim((string)$request->input('name')),$this->context($request),$this->statusValue($request),$this->date($request,'starts_at'),$this->date($request,'ends_at'),$destination,trim((string)$request->input('sponsorship_label','Sponsored'))?:'Sponsored',(int)$request->input('priority',0)]);$id=(int)$pdo->lastInsertId();Database::query("INSERT INTO advertising_creatives (campaign_id,headline,body_text,call_to_action,status,created_at) VALUES (?,?,?,?, 'approved',NOW())",[$id,$headline,trim((string)$request->input('body_text')),trim((string)$request->input('call_to_action'))?:'Learn more']);}
            $pdo->commit();
