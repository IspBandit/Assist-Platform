...
| ID | Outcome | Status | Exit evidence |
| --- | --- | --- | --- |
| COM-001 | Transactional email transport and sender reputation | in progress | Graph fallback and three-brand probes pass; per-brand mailbox selection is implemented behind blank-safe environment settings; create, scope and externally accept the three Exchange shared mailboxes before activation; bounce/suppression acceptance remains |
