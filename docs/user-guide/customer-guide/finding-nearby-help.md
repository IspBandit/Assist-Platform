...
When the directory is still empty or thin for that service and place, VanAssist may show public-source businesses from Google (feature-flagged). Those rows are not verified VanAssist specialists: confirm they can do the work before travelling. If nothing is close, the message says so and gives the approximate distance to the nearest public-source option instead of implying locals are available. Some may also appear later as unclaimed public listings you can open or that a business can claim.
