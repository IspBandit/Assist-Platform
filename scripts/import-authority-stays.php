...
fclose($fh);if($apply){$pdo->commit();}else{$pdo->rollBack();}echo json_encode(['mode'=>$apply?'applied':'dry-run','created'=>$created,'updated'=>$updated,'rejected'=>$rejected],JSON_PRETTY_PRINT)."\n";
