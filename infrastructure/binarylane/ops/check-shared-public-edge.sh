#!/usr/bin/env bash
# Host-owned safety contract for every product attached to the shared public edge.
set -euo pipefail
umask 077
...
test -s "$manifest" || {
  echo 'Required shared-edge site registry is missing or empty.' >&2
  exit 1
}
test -d "$root/sites"
...
inventory() {
  (
    cd "$root"
    find sites -maxdepth 1 -type f -name '*.caddy' -print0 |
      sort -z |
      xargs -0 -r sha256sum
  )
}
...
check_sites() {
  ...
  while IFS='|' read -r url marker extra || [[ -n "${url:-}" ]]; do
    ...
    count=$((count + 1))
    if ! code=$(curl --fail --silent --show-error --location \
      --retry 3 --retry-all-errors --retry-delay 2 \
      --connect-timeout 10 --max-time 20 \
      --user-agent 'Mozilla/5.0 Shared-Edge-Release-Check/1.0' \
      --output "$body" --write-out '%{http_code}' "$url"); then
      rm -f "$body"
      echo "Shared-edge request failed: $url" >&2
      return 1
    fi
    if [[ "$code" != 200 ]] || ! grep -Fqi -- "$marker" "$body"; then
      rm -f "$body"
      echo "Shared-edge status or product identity failed: $url" >&2
      return 1
    fi
    printf 'Healthy: %s\n' "$url"
  done < "$manifest"
  rm -f "$body"
  [[ "$count" -gt 0 ]] || {
    echo 'No registered public sites.' >&2
    return 1
  }
}
...
case "${1:-}" in
  ...
  candidate)
    ...
    docker exec -i "$container" \
      caddy adapt --config /dev/stdin --adapter caddyfile --validate \
      < "$candidate" > "$json"
...
with open(manifest, encoding="utf-8") as stream:
    required = {
        urlsplit(line.split("|", 1)[0]).hostname
        for line in stream
...
if missing:
    raise SystemExit(
        "Candidate drops registered host routes: "
        + ", ".join(sorted(str(host) for host in missing))
    )
...
PY
  ...
  verify)
    ...
    cmp -s "$manifest" "$state/required-sites.txt" || {
      echo 'Site registry changed during release.' >&2
      exit 1
    }
    current=$(mktemp)
    trap 'rm -f "$current"' EXIT
    inventory > "$current"
    cmp -s "$current" "$state/sites.sha256" || {
      echo 'Product-owned edge routes changed during release.' >&2
      exit 1
    }
    check_sites
    ;;
  *)
    echo 'Usage: check-shared-public-edge.sh check|snapshot DIR|candidate CADDYFILE|verify DIR' >&2
    exit 2
    ;;
esac
